...
$("#retrain").click(function () {
    $("#status2").removeClass().toggleClass("fa fa-spinner fa-spin");

    //var jq = $.post('cgi-bin/train.py')
    //var jq = $.post('cgi-bin/traincnn.py')
    var jq = $.post('cgi-bin/traincnn2.py')
        .done(function (json) {
            $("#status2").removeClass().toggleClass("fa fa-check");
            alert('train ok');
        })
        .fail(function (xhr, textStatus, error) {
            $("#status2").removeClass().toggleClass("fa fa-exclamation-triangle");
            console.log("train Error: " + xhr.responseText + ", " + textStatus + ", " + error);
            alert("train Error: " + xhr.responseText + ", " + textStatus + ", " + error);
        });
});
...
$("#predict").click(function(){  
  ...
  //var jq = $.post('cgi-bin/mnistcnn.py', url)
  var jq = $.post('cgi-bin/mnistcnn2.py', url)
    .done(function (json) {
      if (json.result) {
